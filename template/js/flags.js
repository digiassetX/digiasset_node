...
window.flags={
    html:   (countryCode,options={})=> {
        countryCode=countryCode.toUpperCase();
        if (options.class===undefined) {
            options.class='flag flag_' + countryCode;
        } else {
            options.class+=' flag flag_' + countryCode;
        }
        let str = '<div';
        for (let key in options) str += ` ${key}="${options[key]}"`;
        return str + '>&nbsp;</div>';
    },
    name:   (countryCode)=> {
        countryCode=countryCode.toUpperCase();
        for (let {code,name} of countryList) {
            if (code===countryCode) return name;
        }
        throw "Invalid Country Code";
    },
    countries: ()=>JSON.parse(JSON.stringify(countryList))
};