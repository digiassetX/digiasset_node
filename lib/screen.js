...
addIndex("IPFS",yellow+"Initializing"+reset);
...
module.exports.errorLog=errorLog=(text)=>{
    if (logMode) {
        consoleError(text);
    } else {
        lines.push(red+text.toString()+reset);
        if (lines.length > states["Log"] + logSize + 1) lines.splice(states["Log"] + 1, 1);
        draw();
    }
}
module.exports.getStatus=()=>{
    return lines.slice(states["IPFS"],states['Log']-1);
}
