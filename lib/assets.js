...
module.exports.list={
    ...
    subscriptions:()=>{
        let data=[];
        const approved=config.get("approved").list;
        for (let url of approved) {
            data.push({url,approved:true,rejected:false});
        }
        const rejected=config.get("rejected").list;
        for (let url of rejected) {
            let found=false;
            ...
            if (!found) data.push({url,approved:false,rejected:true});
        }
        return data;
    }
};
