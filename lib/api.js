...
app.post('/api/stream/hash/lookup.json',multer().single('file'),async(req,res)=> {
    let response;
    try {
        //get files hash
        ...
        // noinspection JSUnresolvedVariable
        let file=req.file;
        let hex=crypto.createHash('sha256').update(file.buffer).digest('hex');
        response={hex};

        //get the address that will publish the hash
        let {start=1,end}=req.body;
        ...
        for (let i=start;i<=end;i+=1000000) {
            ...
            for (let {height,txid,data} of objectData) {
                if (data===hex) {
                    res.status(200).json({height,txid,hex});
                    return;
                }
            }
        }
...
app.post('/api/wallet/hash/write.json',multer().single('file'),async(req,res)=> {
    ...
    try {
        ...
        if (balance<1000n) throw "Not enough funds";

        //create transaction
        let outputs=[{data: hex}];
        balance-=BigInt(txSize(inputs.length,2,hex.length));
