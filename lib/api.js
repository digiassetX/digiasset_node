...
app.get('/api/config/wallet/autoSet.json',async(req,res)=>{
    try {
        //todo look for digibyte.conf
        //if found and rpc settings are set then use testWalletConfig
        //if rpc settings are not set add random setting and return error "You will need to restart core wallet"
        res.status(200).json(false);    //todo need to actually try in future.  place holder here for now
    } catch (e) {
        res.status(200).json({error:e.toString()});
    }
});
...
app.get('/api/cid/reject.json',async(req,res)=>{
    try {
        const {cid,assetId}=req.body;
        if (assetId===undefined) throw "assetId parameter not set";
        if (cid===undefined) throw "cid parameter not set";
...
app.get('/api/assetId/reject.json',async(req,res)=>{
    try {
        const {cid,assetId}=req.body;
        if (assetId===undefined) throw "assetId parameter not set";
        if (cid===undefined) throw "cid parameter not set";
