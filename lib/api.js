...
/* ___ ___  ___  ___
 ...
 | (_| (_) |   /\__ \
  \___\___/|_|_\|___/
 */
const coreStream=async(url,req,res)=>{
    //verify inputs
...
const coreIPFS=async(cid,req,res)=>{
    res.status(200).end(await ipfs.cat(cid));
}
...
app.post('/api/cors/',async(req,res)=> {
    try {
        ...
        if (scheme==="ipfs") {
            coreIPFS(cid,req,res);
        } else {
            coreStream(url,req,res);
        }
    } catch (e) {
...
app.get('/api/cors/http/*',async(req,res)=> {
    try {
        let url = 'http://'+req.path.substr(16);
        coreStream(url,req,res);
    } catch (e) {
        res.status(200).json({error:e.toString()});
    }
...
app.post('/api/config/subscription/approved.json',(req,res)=>{
    try {
        ...
        subscriptions[url].approved=enabled;
        config.set("subscriptions",subscriptions);
        res.status(200).json(true);
    } catch (e) {
        res.status(200).json({error:e.toString()});
...
app.post('/api/config/subscription/rejected.json',(req,res)=>{
    try {
        ...
        let subscriptions=config.get("subscriptions");
        if (subscriptions[url]===undefined) subscriptions[url]={approved:false,rejected:false};
        subscriptions[url].rejected=enabled;
        config.set("subscriptions",subscriptions);
        res.status(200).json(true);
    } catch (e) {
        res.status(200).json({error:e.toString()});
 ...
 */
app.get('/api/stream/clearCache.json',async(req,res)=>{
    try {
        await stream.clearCache();
        res.status(200).json(true);
    } catch (e) {
        res.status(200).json({error:e.toString()});
    }
...
app.get('/api/wallet/asset/list.json',async(req,res)=>{
    ...
    try {
        if (!byLabel) {
            ...
            response=await wallet.getAssets(addresses);
            ...
            for (let label of labels) {
                ...
                let assets=await wallet.getAssets(addresses);
                for (let {assetId,value,decimals,cid,cache} of assets) response.push({label,assetId,value,decimals,cid,cache});
            }
...
app.get('/api/wallet/asset/issuable.json',async(req,res)=>{
    ...
    try {
        ...
        //see if kyc verified
        let addressCache=config.get("addresscache");
        for (let address of addresses) {
            //if address data not already cached get it
            if (addressCache[address]===undefined) {
                try {
                    let {kyc, issuance=[]} = await stream.json(address);
                ...
                config.set("addresscache", addressCache);
            }

            //check if should be in this list
            if (addressCache[address].kyc!==kyc) continue;

            //add to response
            response.push({
                address,
                issuance: addressCache[address].issuance
            });
        }
...
app.get('/api/wallet/asset/:assetId.json',async(req,res)=>{
    ...
    try {
        ...
        //make sure in cache
        response=JSON.parse(JSON.stringify(await wallet.cacheAsset(assetId)));//deep copy
...
app.post('/api/wallet/addresses/new.json',async(req,res)=>{
    ...
    try {
        const {label}=req.body;
        response=await wallet.getNewAddress(label,"bech32");
    } catch (e) {
        response={error:e.toString()};
    }
    res.status(200).json(response);
    ...
    let response=await(async(req)=> {
        try {
            ...
            let {
                ...
                tx,
                changes
            } = await stream.assetEncoder.transfer(utxos, assetOutputs, {assetChange, coinChange});
            ...
            let hex = await wallet.createRawTransaction(inputs, outputs);
            let costs = [];
            ...
            //count amount of assets sent
            costs.push({type: "Assets sent", amount: wallet.satToDecimal(needed, decimals) + " Assets"});
...
app.post('/api/wallet/kyc.json',async(req,res)=>{
    ...
    try {
        ...
        response='https://digiassetx.com/kyc-address/?data='+Buffer.from(JSON.stringify(pageVars)).toString('base64');
    } catch (e) {
