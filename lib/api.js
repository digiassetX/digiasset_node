        ...
        if (!byLabel) {
            ...
            for (let label of labels) {
                ...
                for (let {assetId,value,decimals,cid,data} of assets) {
                    let cidToUse=cid||data.metadata.pop().cid;
                    let metadata;
                    try {
...
app.post('/api/wallet/build/assetIssuance.json',async(req,res)=>{
    let response;
    try {
        let {recipients,address,options,metadata}=req.body;

        //get list of usable utxos(remove all utxos with less then 1000sat)
        let utxos = (await wallet.listUnspent(1,99999999999,[address])).filter(({value})=>BigInt(value)>=1000);

        //get change addresses
        let coinChange = await wallet.getChangeAddress();
        options.changeAddress=coinChange;

        //modify output to match
        let {
            inputs,
            outputs
        } = await stream.assetEncoder.issuance(utxos, recipients,metadata,options);
        ...
        for (let address in outputs) {
            ...
            let sats=BigInt(Math.round(parseFloat(outputs[address])*100000000));
            if (sats===600n) coloring+=sats;
            if (address==="dgb1qjnzadu643tsfzjqjydnh06s9lgzp3m4sg3j68x") permanent=sats;
            miningFee-=sats;
        }
        costs.push({type: "Mining Fees", amount: wallet.satToDecimal(miningFee, 8) + " DGB"});
        costs.push({type: "Asset Output Creation", amount: wallet.satToDecimal(coloring, 8) + " DGB"});
        costs.push({type: "Permanent Data Storage", amount: wallet.satToDecimal(permanent, 8) + " DGB"});
