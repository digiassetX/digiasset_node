...
app.get('/api/wallet/assets.json',async(req,res)=>{
    let response;
    try {
...
app.get('/api/wallet/assetsbylabel.json',async(req,res)=>{
    let response=[];
    try {
...
app.post('/api/wallet/build/assetTx.json',async(req,res)=>{
    try {
        const {recipients,label,assetId}=req.body;

        //get quantity needed
        let needed=0n;
        for (let address in recipients) needed+=BigInt(recipients[address]);
        ...
        let {inputs,outputs,royalties,tx,changes}=await stream.assetEncoder.transfer(utxos,assetOutputs,{assetChange,coinChange});
        ...
        if (assetId!=="DigiByte") {
            ...
            if (burntAssets>0n) costs.push({type:"Burnt assets",amount:wallet.satToDecimal(burntAssets,decimals)+" Assets"});
        }

        //count amount of assets sent
        costs.push({type:"Assets sent",amount:wallet.satToDecimal(needed,decimals)+" Assets"});

        res.status(200).json({costs,hex});
    } catch (e) {
        res.status(200).json({costs:[{type: e.toString(),amount:""}],hex:""});
    }
});
