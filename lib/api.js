...
app.get('/api/status/console.json',(req,res)=>{
    res.status(200).json(screen.getStatus());
});
app.get('/api/status/ip.json',async(req,res)=>{
    res.status(200).json(await config.ip());
});
...
const downloadVersion=async(version)=>{
    ...
    //download file
    try {
        if (version===undefined) throw "No Template Available";    //no version available
...
app.post('/api/wallet/build/assetIssuance.json',async(req,res)=>{
    ...
    try {
        ...
        if (PERMANENT_ADDRESS[tax]===undefined) throw "Invalid tax location";
