 ...
 * @return {Promise<string[]>}
 */
const getAddresses=async(label)=>{
    ...
    for (let label of labels) {
        ...
        for (let address in data) {
            if (data[address]["purpose"]==="receive") addresses.push(address);
        }
    }
    return addresses;
}
 ...
 */
const getAssets=async(addresses)=>{
    ...
    for (let address of addresses) {
        try {
            let utxos=JSON.parse(await streamToString(await getStream(address + "_utxos")));
            for (let {value, assets} of utxos) {
                dgb+=BigInt(value);
                if (assets!==undefined) {
                    for (let {assetId,amount,decimals,cid,rules=false} of assets) {
                        if (totalAssets[assetId+cid]===undefined) totalAssets[assetId+cid]={assetId,value:0n,decimals,cid,rules};
    ...
    //order as array
    let data=[{assetId:"DigiByte",value:dgb.toString(),decimals:8,rules:false}];
    for (let index in totalAssets) {
        let {assetId,value,decimals,cid,rules}=totalAssets[index];
        data.push({assetId,value: value.toString(),decimals,cid,rules});
    }
    return data;
}
...
app.get('/api/wallet/assets.json',async(req,res)=>{
    try {
        ...
        res.status(200).json(await getAssets(addresses));
    } catch (e) {
        res.status(200).json({error:e.toString()});
    }
});
...
app.get('/api/wallet/assetsbylabel.json',async(req,res)=>{
    try {
        ...
        for (let label of labels) {
            ...
            let assets=await getAssets(addresses);
            for (let {assetId,value,decimals,cid} of assets) data.push({label,assetId,value,decimals,cid});
        }
        res.status(200).json(data);
    } catch (e) {
        res.status(200).json({error:e.toString()});
...
app.post('/api/wallet/addresses/list.json',async(req,res)=>{
    ...
    try {
        ...
        addresses=await getAddresses(label);
    } catch (e) {
        res.status(200).json({error:e.toString()});
        return;
    }
...
app.post('/api/wallet/build/assetTx.json',async(req,res)=>{
    try {
        ...
        //todo this is fake
        let costs=[
            {type:"mining fees",amount:Math.random().toFixed(8)+" DGB"}
        ];
        await sleep(2000);

        res.status(200).json({costs,hex:"asdad"});
    } catch (e) {
        res.status(200).json({error:e.toString()});
    }
});
...
app.post('/api/wallet/send.json',async(req,res)=>{
    try {
        ...
        res.status(200).json(await wallet.sendRawTransaction(signedHex));
    } catch (e) {
        res.status(200).json({error:e.toString()});
    }
});
