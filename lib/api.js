...
const apiVersion=4;    //if changing interface in this file update /template/js/api.js to match
const apiMinVersion=3;
const DUST=1000n;
const MAX_COUNT_FIT=10;
const FILE_STORAGE_FEE=10000n;
...
const metaProcessor=require('./meta_processor');
...
const processError=(e)=>{
    if (e.name!=="ExpectedError") fs.appendFileSync('_error.apiCall.log',e.stack+"\r\n-------------------------------\r\n");
    return (e.message!==undefined)?{error:e.message}:{error:e.toString()};
}
...
app.get('/api/status/port.json',async(req,res)=>{
    let response;
    let peerId
    try {
        peerId=await config.peerId();
        response=peerId.match(/tcp\/([0-9]*)\//)[1];
    } catch (e) {
        response=processError(e);
    }
    res.status(200).json(response);
...
app.post('/api/user/login.json',session,async(req,res)=>{
    ...
    try {
        if (mainConfig.users !== false) {
            ...
            if (code !== mainConfig.users[user].hash) throw new ExpectedError("Invalid Password");
...
/* ___ ___  ___  ___
 ...
 | (_| (_) |   /\__ \
  \___\___/|_|_\|___/
 */
const corsStream=async(url,req,res,hash=undefined)=>{
    //verify inputs
    ...
    //if hash not included then stream direct
    if (hash===undefined) {
    ...
    //download file and check its hash
    let fileData=(await got.get(url,{encoding: "binary"})).rawBody;
    let myHash=crypto.createHash('sha256').update(fileData).digest('hex');
    if (myHash===hash.toLowerCase()) return res.status(200).end(fileData);

    //invalid hash so return the error image
    let stream = fs.createReadStream("template/images/corrupt_image.png");
    stream.pipe(res);
    stream.on('error', (_) => {
        screen.log(url + " failed to load");
    });
...
app.get('/api/cors/https/*',async(req,res)=> {
    try {
        let hash;
        let urlParts=req.path.substr(16);
        if (urlParts.substr(0,7)==="sha256-") {
            hash=urlParts.substr(7,64);
            urlParts=urlParts.substr(72);
        }
        let url = 'https://'+urlParts;
        // noinspection ES6MissingAwait
        corsStream(url,req,res,hash);
    } catch (e) {
        res.status(200).json(processError(e));
...
app.get('/api/cors/http/*',async(req,res)=> {
    try {
        let hash;
        let urlParts=req.path.substr(15);
        if (urlParts.substr(0,7)==="sha256-") {
            hash=urlParts.substr(7,64);
            urlParts=urlParts.substr(72);
        }
        // noinspection HttpUrlsUsage
        let url = 'http://'+urlParts;
        // noinspection ES6MissingAwait
        corsStream(url,req,res,hash);
    } catch (e) {
        res.status(200).json(processError(e));
...
app.get('/api/cid/:cid.html',async(req,res)=>{
    ...
    let cidData;
    try {
        let a=await ipfs.checkPinned(cid);
        console.log(a);
    ...
    try {
        //see if it is json
        let jsonData=JSON.parse(cidData);
        let html='<html lang="en"><head><meta name="viewport" content="width=device-width, initial-scale=1"><title>'+cid+'</title>';
        for (let path of cidPagesCSS) html+=`<link rel="stylesheet" href="${path}" type="text/css">`;
        html+='</head><body><div id="pre"></div>';

        //check if generic JSON data or DigiAsset
        if (
            (jsonData.data===undefined)||
            (jsonData.data.assetName===undefined)||
            (jsonData.data.description===undefined)||
            (jsonData.data.urls===undefined)
        ) {
            //generic JSON
            let str=JSON.stringify(jsonData, null, 4);
            html+=`<div class="json">${str}</div>`;
        } else {
            //Asset Meta Data
            html+=metaProcessor(jsonData);
        }

        html+='<div id="post"></div>';
        for (let path of cidPagesScripts) html+=`<script src="${path}"></script>`;
        html+='</body></html>';
    ...
    if (/^[\x00-\x7F]*$/.test(cidData)) {
        let html=cidData
            ...
            .replace(/"/g, '&quot;')
            .replace(/\n/g,'<br>');
        html='<html lang="en"><head><meta name="viewport" content="width=device-width, initial-scale=1"><link href="/css/main.css" rel="stylesheet"><title>'+cid+'</title></head><body>'+html+'</body></html>';
        res.status(200).end(html);
        return;
    }
...
app.post('/api/stream/clear.json',async(req,res)=>{
    let response=true;
    try {
        let {full=false}=req.body;
        await stream[full?"fullCacheClear":"fastClear"]();
    } catch (e) {
        response=processError(e);
    }
    res.status(200).json(response);
...
app.post('/api/stream/hash/lookup.json',async(req,res)=> {
    const indexMultiple=100000;
    let response={};
    try {
        //get the address that will publish the hash
        let {start=1,end,hashes}=req.body;
        if (end===undefined) end=await stream.json("height");
        start=Math.floor(start/indexMultiple)*indexMultiple;
        end=Math.ceil(end/indexMultiple)*indexMultiple;

        //search for data in chain
        for (let i=start;i<=end;i+=indexMultiple) {
            let objectData=await stream.json("data_tx_"+i);
            if (objectData===false) continue;
            for (let {height,txid,data} of objectData) {
                if (hashes.indexOf(data)!==-1) {
                    res.status(200).json({height,txid,hash:data});
                    return;
                }
            }
        }
...
app.post('/api/wallet/build/assetIssuance.json',async(req,res)=>{
    ...
    try {
        ...
        while (recipientArray.length>0) {
            ...
            issuer.clearPermanentProviders();
            issuer.addPermanentProvider(PERMANENT_ADDRESS[tax],true);
...
app.post('/api/wallet/hash/write.json',multer().single('file'),async(req,res)=> {
    ...
    try {
        ...
        let balance=-FILE_STORAGE_FEE;
        let inputs=[];
        for (let utxo of utxos) {
            if (utxo.assets!==undefined) continue;
            let value=BigInt(utxo.value);
            if (value<1000n) continue;
            balance+=value;
            inputs.push(utxo);
            if (balance>=FILE_STORAGE_FEE) break;
        }
        if (balance<FILE_STORAGE_FEE) throw new ExpectedError("Not enough funds");
        ...
        response=await wallet.send(rawHex,password);
    } catch (e) {
        response=processError(e);
    }
    res.status(200).json(response);
...
app.post('/api/file/hash.json',multer().single('file'),async(req,res)=> {
    ...
    try {
        ...
        response={
            ...
            sha1:crypto.createHash('sha1').update(file.buffer).digest('hex'),
            sha256:crypto.createHash('sha256').update(file.buffer).digest('hex'),
            sha384:crypto.createHash('sha384').update(file.buffer).digest('hex'),
            sha512:crypto.createHash('sha512').update(file.buffer).digest('hex')
        };
    } catch (e) {
        response=processError(e);
    }
