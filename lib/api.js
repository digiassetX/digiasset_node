...
app.get('/api/cid/:cid.html',async(req,res)=>{
    ...
    try {
        ...
        html='<html lang="en"><head><title>'+cid+'</title>';
        for (let path of cidPagesCSS) html+=`<link rel="stylesheet" href="${path}" type="text/css">`;
        html+='</head><body><div id="pre"></div><h1>Meta Data:</h1><div class="json">';
        let lastImage={};
        for (let line of lines) {


            //Description processing routine
            if (line.startsWith('        "description": "')) {
                let description = line.substr(23);
                let index = description.lastIndexOf('"');
                let ending = description.substr(index+1);
                description=JSON.parse(description.substr(0,index+1));
                description = description.substr(0, index)
                    ...
                    .replace(/"/g, '&quot;')
                    .replace(/\n/g, "<br>");
                html += `        "description": </div><div class="description">${description}</div><div class="json">        ${ending}\n`;
                continue;
            }

            //Data URL processing routines
            ...
            //Image/Video Processing routine
            ...
            if ((endSearch==="}")||(endSearch==="},")) {
                ...
                if ((lastImage.mimeType!==undefined)&&(lastImage.url!==undefined)) {
                    switch (lastImage.mimeType) {
                        ...
                        case "image/png":
                        case "image/gif":
                            html += `</div><img src="${lastImage.url}" class="asset_data_image" alt="${lastImage.url}"><div class="json">`;
                            break;
                        ...
                        case "video/mp4":
                            html += `</div><video class="asset_data_video" controls><source src="${lastImage.url}"></video><div class="json">`
                            break;

                        default:
                            html += `${"".padStart(line.length+3," ")}<a href="${lastImage.url}" target="_blank">Open</a>\r`
                    }
                }
