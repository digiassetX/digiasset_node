...
app.post('/api/wallet/build/assetIssuance.json',async(req,res)=>{
    ...
    try {
        ...
        while (recipientArray.length>0) {
            ...
            let lastCount;
            while ((counts<=MAX_COUNT_FIT)&&(recipientArray.length>0)) {
            ...
            for (let vout in outputs) {
                ...
                if (address === issuingAddress) {
                    let {txid}=wallet.decodeRawTransaction(txHex);
                    utxos=[{txid,vout}];    //todo probably need to add value data since decoder cant look up on its own
                }
                miningFee -= sats;
            }
