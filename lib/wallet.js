...
module.exports.satToDecimal=(value,decimals)=>{
    ...
    return negative+str.substr(0,periodPoint)+'.'+str.substr(periodPoint);
}
...
const getWallet=async()=>{
    ...
    if (wallet===undefined) throw new ExpectedError("Wallet not set up");
    ...
    try {
        ...
        throw new ExpectedError("Wallet offline or config has changed");
    }
    return dgbWallet;
}
...
let syncChangeAddresses=async()=>{
    ...
    if (syncChangeAddressesSyncing.length>0) {
        screen.log("Processing waiting tasks");
        while (syncChangeAddressesSyncing.length > 0) syncChangeAddressesSyncing.pop()();
        screen.log("Finished waiting tasks");
    }
...
module.exports.findAsset=async(assetId,label)=>{
    ...
    throw new ExpectedError("Asset not found");
}
...
module.exports.getSignature=async(address,message,password="")=> {
    let wallet=await getWallet();
    if (password !== "") await wallet.walletPassPhrase(password, 60);
