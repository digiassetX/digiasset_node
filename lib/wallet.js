 ...
 * }}
 */
module.exports.cacheAsset=cacheAsset=async(assetId)=>{
    let assetcache=config.get("assetcache");
    if (assetcache[assetId]===undefined) {
        //lookup asset data
        let {rules,kyc,issuer,divisibility,metadata,aggregation,locked}=await stream.json(assetId);
        if (rules!==undefined) rules=rules.pop();   //only keep most recent rules

        //save
        assetcache[assetId]={rules,kyc,issuer,divisibility,metadata,aggregation,locked};
            ...
            for (let {value, assets} of utxos) {
                ...
                if (assets!==undefined) {
                    for (let {assetId,amount,decimals,cid,rules=false} of assets) {
                        //get extra data
                        let cache=await cacheAsset(assetId);

                        //store
                        if (totalAssets[assetId+cid]===undefined) totalAssets[assetId+cid]={assetId,value:0n,decimals,cid,rules,cache};
