...
const lookForNewAssets=async()=>{
                    ...
                    if (data.urls !== undefined) {
                        ...
                        for (let {name, mimeType, url} of data.urls) {
                            ...
                            if (mainConfig.includeMedia !== true) {
                                //check types
                                if ((mainConfig.includeMedia.paid==="always")&&(permanent.assets[assetId][hash].indexOf(mediaCid)!==-1)) {
                                    //passed no code here
                                } else {
    ...
    try {
        ...
        const handleAssetUpdate=async(futures)=>{
            ...
            let passHeights=[];
            for (let {pass,height} of results) {
                if (pass) {
        ...
        if (lowestFail<Infinity) throw new ExpectedError("failed to pin all assets");
...
const checkPort=async()=>{
    ...
    if ((await got.post('https://ipfs.digiassetX.com/reachable.json',{
        ...
        screen.yellow("IPFS Desktop","Port Blocked");
    }
}
