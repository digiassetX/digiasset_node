...
//try to use UPnP to route data to this machine
const natUpnp = require('nat-upnp');
const natClient = natUpnp.createClient();
const natForward=async(port)=>{
    try {
...
/**
 * Trys to connect to other known nodes
 */
const connectToPeers=()=>{
    try {
        got.get(`https://ipfs.digiassetx.com/nodes.json`, {
            ...
            let nodeCount=data.length-1;
            for (let {id} of data) {
                if (id===myself) continue;  //don't try to connect to self
                if (Math.random()*nodeCount>8) continue;   //randomly connect to some nodes
                try {
...
const lookForNewAssets=async()=>{
    ...
    try {
        ...
        for (let assetId in data) {
            ...
            if (futures.length>5) await handleAssetUpdate(futures);
        }
...
const makeSureIPFSrunning=async()=>{
    while (true) {
        try {
            ...
            screen.red("IPFS","Not Responding");
            await sleep(10000);
        }
    }
}
...
const checkPort=async()=>{
    ...
    await natForward(port);
    if ((await got.post('https://ipfs.digiassetX.com/reachable.json',{
        responseType:   "json",
        json:           {peerId}
    })).body) {
        screen.green("IPFS","Running on port "+port);
        portReachable=true;
    } else {
        screen.yellow("IPFS",`Port ${port} Blocked`);
    }
}
...
(async()=> {
    ...
    //make sure DigiByte Logo & meta data is pinned
    screen.yellow("IPFS","Attempting to Download DigiByte Logo");
    try {
        connectToPeers();
        await ipfs.pinAdd("QmSAcz2H7veyeuuSyACLkSj9ts9EWm1c9v7uTqbHynsVbj",300000);
        await ipfs.pinAdd("QmfSVLAntanDUKrEHUnTXRh53GLUBHFfxk5x6LH4zz9PM4",300000);
    } catch (e) {}
