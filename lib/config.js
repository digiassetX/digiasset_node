...
configs["main"]=read("main.json",{
    ignoreList: [                   //list of cids that come with IPFS
    ],
    ...
    timeout:        1800000,        //max number of ms to look for an ipfs object.  Default 30min
    ...
    publish:        false,
    ipfs:           true            //true to use internal ipfs node.  integer
});
...
module.exports.peerId=async()=>{
    try {
        ...
        let {addresses,Addresses} = await futureId;
        ...
        if (possible.length===0) {
            //we didn't find outside line so lets try constructing ourselves
            for (let id of addresses) {
