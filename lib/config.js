...
configs["approved"]=read("approved.json", {cid:[],assetId:[],list:[],cache:[]});
configs["rejected"]=read("rejected.json",{cid:[],assetId:[],list:[],cache:[]});
configs["assets"]=read("assets.json",[]);
configs["assetcache"]=read("assetcache.json",{});
configs["addresscache"]=read("addresscache.json",{});
...
if (Array.isArray(configs["approved"])) {
    ...
    configs["approved"]={cid,assetId:[],list:[],cache:[]};//cache will get reprocessed after next sync
    write('approved.json',configs["approved"]);
}
if (Array.isArray(configs["rejected"])) {
    upgrading=true;
    let cid=configs["rejected"];
    configs["rejected"]={cid,assetId:[],list:[],cache:[]};//cache will get reprocessed after next sync
